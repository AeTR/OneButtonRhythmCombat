...
public class EnemySpawner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(FencerSwordController.me.space && roundEnd && GameManager.me.timer > GameManager.me.timerLimit)
        {
            round++;
            roundActive = true;
            roundEnd = false;
    ...
    void FixedUpdate()
    {
        ...
        if (round > -1 && roundActive)
        {
            ...
            if(nextBeat == beats[dataInt])
            {
                ...
                {
                    liveEnemies.Add(Instantiate(dog, transform.position - (Vector3.up * 0.1f), transform.rotation));
                }
                ...
                {
                    liveEnemies.Add(Instantiate(heart, transform.position + (Vector3.up * 0.5f), transform.rotation));
                }
